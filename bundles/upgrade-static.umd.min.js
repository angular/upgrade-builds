 ...
 */
function noNg(){throw new Error("AngularJS v1.x is not loaded!")}function setAngularLib(ng){angular=ng}function getAngularLib(){return angular}function bootstrap(e,modules,config){angular.bootstrap(e,modules,config)}function module$1(prefix,dependencies){return angular.module(prefix,dependencies)}function element(e){return angular.element(e)}/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
 ...
 * found in the LICENSE file at https://angular.io/license
 */
function downgradeInjectable(token){var factory=function(i){return i.get(token)};return factory.$inject=[INJECTOR_KEY],factory}function getOrCall(property){return isFunction(property)?property():property}function isFunction(value){return"function"==typeof value}function isMap(value){return value&&!Array.isArray(value)&&"object"==typeof value}function setTempInjectorRef(injector){tempInjectorRef=injector}function injectorFactory(){var injector=tempInjectorRef;return tempInjectorRef=null,injector}function rootScopeFactory(i){return i.get("$rootScope")}function compileFactory(i){return i.get("$compile")}function parseFactory(i){return i.get("$parse")}var angular;try{window.hasOwnProperty("angular")&&setAngularLib(window.angular)}catch(e){setAngularLib({bootstrap:noNg,module:noNg,element:noNg,version:noNg,resumeBootstrap:noNg,getTestability:noNg})}/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
