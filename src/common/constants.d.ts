/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
 * found in the LICENSE file at https://angular.io/license
 */
export declare const $COMPILE: string;
export declare const $CONTROLLER: string;
export declare const $DELEGATE: string;
export declare const $HTTP_BACKEND: string;
export declare const $INJECTOR: string;
export declare const $PARSE: string;
export declare const $PROVIDE: string;
export declare const $ROOT_SCOPE: string;
export declare const $SCOPE: string;
export declare const $TEMPLATE_CACHE: string;
export declare const $TEMPLATE_REQUEST: string;
export declare const $$TESTABILITY: string;
export declare const COMPILER_KEY: string;
export declare const GROUP_PROJECTABLE_NODES_KEY: string;
export declare const INJECTOR_KEY: string;
export declare const NG_ZONE_KEY: string;
export declare const REQUIRE_INJECTOR: string;
export declare const REQUIRE_NG_MODEL: string;
export declare const UPGRADE_MODULE_NAME: string;
